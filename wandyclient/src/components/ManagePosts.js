...
function ManagePosts({ clickPosts }) {
  ...
  const [checkedState, setCheckedState] = useState(
    Array(stateLinks.length).fill(false)
  );
  ...
  const handleCheckbox = (position) => {
    ...
    const updCheckedState = checkedState.map((booleanValue, index) =>
      index === position ? !booleanValue : booleanValue
    );
    setCheckedState(updCheckedState);

    if (updCheckedState[position] === true) {
      tagArray.push({
        id: links[position].id,
        name: links[position].name,
        count: links[position].count,
      });
  ...
  const submitCreateForm = async () => {
    const tagNameArray = postTag.map((posttag) => posttag.name);
  ...
  return (
    <>
      {clickPosts && (
        <article className="content">
          <div className="choosecategory">
            <span
              style={createPost ? active : inactive}
              onClick={() => setActive("create")}
            >
              Create A New Post
            </span>
            <span
              style={updatePost || editPost ? active : inactive}
              onClick={() => setActive("update")}
            >
              Update A Post
            </span>
            <span
              style={deletePost ? active : inactive}
                ...
                <tbody>
                  {posts.map((post, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <i
                          style={{ color: "#dba11c", marginRight: "20px" }}
          ...
          {editPost ? (
            <div className="showcategory posts">
              ...
              <img
                src={`http://localhost:5000/${postedImage}`}
                alt="post"
      ...
      <Modal
        ...
        postId={postId}
        postTag={postTag}
        handleDeleteButton={() => handleDeleteButton(postId, postTag)}
      />
    </>
  );
}
